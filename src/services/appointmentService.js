...
async function cancelAppointment(appointmentId) {
  ...
  // Fire-and-forget: notify the next waitlist candidate for this clinic/date.
  // Required lazily to avoid a circular require (waitlistService never
  // imports appointmentService back).
  const waitlistService = require('./waitlistService');
  waitlistService.notifyNextCandidate(appointment).catch((err) => {
    require('../utils/logger').error({ err, appointmentId }, 'Failed to notify waitlist after cancellation');
  });
